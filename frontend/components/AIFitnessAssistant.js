...
export default function AIFitnessAssistant() {
  ...
  // Separate effect for handling the greeting
  useEffect(() => {
    if (userProfile && chatMessages.length === 0) {
      const greeting = generateInitialGreeting(userProfile, userStats);
      setChatMessages([
        {
          type: "ai",
          content: greeting,
  ...
  }, [userProfile, userStats, chatMessages.length]);
...
I can help you with workouts, nutrition advice, and tracking your progress. What would you like to know?`;
  ...
  const getMessageIcon = (content) => {
    ...
    if (contentLower.includes("workout") || contentLower.includes("exercise"))
      return "💪";
    if (contentLower.includes("diet") || contentLower.includes("nutrition"))
      return "🥗";
    if (contentLower.includes("motivation") || contentLower.includes("mindset"))
      return "🔥";
    ...
    try {
      ...
      setChatMessages((prev) => [
        ...prev,
        {
          type: "ai",
          content:
            "I'm having trouble processing your request right now. Try again later!",
          timestamp: new Date(),
        },
      ]);
    } finally {
  ...
  return (
    <div className="bg-purple-500/5 border border-purple-500/10 rounded-xl p-4">
      ...
      <div
        ...
        className="mb-4 space-y-4 max-h-60 overflow-y-auto scrollbar-thin scrollbar-thumb-purple-500/20"
      >
        {chatMessages.length === 0 ? (
          <p className="text-sm text-gray-400">
            Hi! Ask me anything about workouts, nutrition, or fitness tips! 💪
          </p>
        ) : (
          chatMessages.map((message, index) => (
            ...
            >
              <div
                className={`max-w-[80%] rounded-lg p-3 ${message.type === "user" ? "bg-purple-500/10 border border-purple-500/20" : "bg-violet-500/10 border border-violet-500/20"}`}
              >
                {message.type === "ai" && (
      ...
      <form onSubmit={handleChatSubmit} className="relative">
        <input
          ...
          className="w-full bg-purple-500/5 border border-purple-500/20 rounded-lg px-4 py-2 text-sm text-white placeholder-gray-500 focus:outline-none"
        />
        <button
          type="submit"
